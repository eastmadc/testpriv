...
# find out where we're rooted without any '..' which
# are annoying when passed around (especially via
# INCVPATH).

IOPKT_ROOT:= $(CURDIR)
define trimdir
        IOPKT_ROOT:= $(patsubst %/,%, $(dir $(1)))
endef
$(foreach iter, $(subst /, $(space), $(dir $(IOPKT_MK))), \
    $(eval $(call trimdir, $(IOPKT_ROOT))))


ifeq ($(filter Linux QNX Darwin,$(shell uname -s)),)
ISWIN= yes
else
ISWIN= no
endif
