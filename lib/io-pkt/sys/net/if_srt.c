...
#ifdef __QNXNTO__
static int srt_ioctl(struct file *, u_long, void *, struct lwp *);
static int srt_close(struct file *, struct lwp *);

	                                                

static const struct fileops srt_fileops = {
	.fo_read = fnullop_read,
	.fo_write = fnullop_write,
	.fo_ioctl = srt_ioctl,
...
static int
srt_if_ioctl(struct ifnet *ifp, u_long cmd, caddr_t data)
{
	struct ifaddr *ifa;
	int s;
	int err;
...
#else
int
srt_open(struct lwp *l, struct msg_open_info *mop, struct file **retval)
{
	struct proc	*p;
	int		error, unit;
	struct file	*fp;
...
srt_close(
#ifndef __QNXNTO__
    dev_t dev, int flag, int mode, struct lwp *l
#else
    struct file *fp, struct lwp *lwp
    )
#endif
{
#ifndef __QNXNTO__
...
#else
    struct file *fp, u_long cmd, void *data, struct lwp *lwp
#endif
    )
{
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/lib/io-pkt/sys/net/if_srt.c $ $Rev: 680336 $")
#endif
