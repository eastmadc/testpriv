...
#if !defined(__QNXNTO__) || defined(CRYPTO_TIMING)
static	int crypto_timing = 0;
#endif
#ifdef __QNXNTO__
void	crypto_intr(void *);
struct	ifqueue crypto_intrq = { 0 };
static void * crypto_dequeue(struct ifqueue *);
#endif
...
#else
	crypto_intrq.ifq_intr = crypto_intr;
	crypto_intrq.ifq_next = &crypto_intrq;
	crypto_intrq.ifq_prev = &crypto_intrq.ifq_next;
	crypto_intrq.ifq_maxlen = IFQ_MAXLEN;
	/*
	 * We don't queue mbufs and therefore don't use IF_ENQEUE() to queue.
	 */
	crypto_intrq.ifq_dequeue = crypto_dequeue;
#endif
...
#else
			/*
			 * TODO: Look into this later, but do we need this?
			 * OpenBSD doesn't have this function, only NetBSD and
			 * (maybe, don't know) FreeBSD. Also, on NetBSD, the
			 * hifn driver doesn't even call this! Looks like a bug
			 * to me.
			 *
			 * Also noticed that when dispatch fails because all of
			 * the drivers are busy, softintr is scheduled. Well,
			 * if softintr runs and all drivers are still busy,
			 * this function needs to get called  to allow progress
			 * on queued requests. Hifn driver doesn't even call
			 * this function! These comments only apply to NetBSD,
			 * because softintr doesn't check for case where
			 * nothing got passed to drivers (In FreeBSD, the
			 * kthread doing this does check the case where nothing
			 * gets passed to drivers).
			 */
#endif
		}
		err = 0;
...
#else
			/*
			 * TODO: This option isn't used by the stack, so no
			 * support for it yet. Will come back to it later.
			 */
#endif
		}
...
#else
	struct ifqueue *inq = &crypto_intrq;

	NW_SIGLOCK(&stk_ctl.pkt_ex, iopkt_selfp);
	if (crp->crp_onqueue == 1) {
		goto OUT;
	}
	if (IF_QFULL(inq)) {
		IF_DROP(inq);
		/* TODO: what here? Used to do something like m_freem */
	} else {
		struct cryptop **head, **tail;

		head =(struct cryptop **)&inq->ifq_head;
		tail =(struct cryptop **)&inq->ifq_tail;
...
OUT:
	NW_SIGUNLOCK(&stk_ctl.pkt_ex, iopkt_selfp);
 ...
 */
static void *
crypto_dequeue(struct ifqueue *ifq)
{
	struct cryptop *crp, **head, **tail;

	head =(struct cryptop **)&ifq->ifq_head;
	tail =(struct cryptop **)&ifq->ifq_tail;

	crp = *head;

	if ((*head = crp->crp_inext) == NULL) {
		*tail = NULL;
	}
	crp->crp_inext = NULL;
	crp->crp_onqueue = 0;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/lib/io-pkt/sys/opencrypto/crypto.c $ $Rev: 776049 $")
#endif
