...
#ifdef __QNXNTO__
		case CRYPTO_AES_XCBC_MAC:
			axf = &swcr_auth_hash_cipher_aes_xcbc_mac;
			goto auth4common;
		auth4common:
			(*swd)->sw_ictx = malloc(axf->auth_hash->ctxsize,
				M_CRYPTO_DATA, M_NOWAIT);
			...
			error = axf->setkey((*swd)->sw_ictx, cri->cri_key, cri->cri_klen / 8);
			if (error) {
				swcr_freesession(NULL, i);
				return error;
			}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/lib/io-pkt/sys/opencrypto/cryptosoft.c $ $Rev: 680336 $")
#endif
