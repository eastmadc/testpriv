...
#else
	/*
	 * Update the received interface....
	 * Look up if_inner instead of referencing ifn_inner to avoid stale pointers.
	 */
	if (sav->if_inner != NULL && (ifp = ifunit(sav->if_inner,ANY_FIB))) {
		ifp->if_input(ifp, m);
		/*XXX: ipsec i/f may want a check to ensure this is ipsec interface...*/
	} else if (ipsec_globalifp != NULL) { /* XXX Backward compatibility */
		DPRINTF(("ipsec4_common_input_cb: changing rcvif from %s to %s\n",
			m->m_pkthdr.rcvif->if_xname, ipsec_globalifp->if_xname));
		m->m_pkthdr.rcvif = ipsec_globalifp;
		schednetisr(NETISR_IP);
	}
	else
		schednetisr(NETISR_IP);
...
#if defined(__QNXNTO__) && defined(PFIL_HOOKS)
		/*
		 * If this is not going back to the ip(6)_input functions then
		 * do the pfil hooks here if needed.
		 */
		if (pfil_ipsec &&
		    ((inet6sw[ip6_protox[nxt]].pr_flags & PR_LASTHDR) != 0) &&
		    (nxt != IPPROTO_IPV6) && (nxt != IPPROTO_IPV4)) {
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/lib/io-pkt/sys/netipsec/ipsec_input.c $ $Rev: 831471 $")
#endif
