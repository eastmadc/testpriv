...
#else
	if (m != NULL) {
		sav = KEY_ALLOCSA(&tc->tc_dst, tc->tc_proto, tc->tc_spi, 0, 0, m->m_pkthdr.rcvif);
	} else {
		/* Shouldn't happen... */
		ahstat.ahs_crypto++;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/lib/io-pkt/sys/netipsec/xform_ah.c $ $Rev: 881343 $")
#endif
