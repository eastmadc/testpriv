...
#else
	if (m != NULL) {
		sav = KEY_ALLOCSA(&tc->tc_dst, tc->tc_proto, tc->tc_spi, sport, dport, m->m_pkthdr.rcvif);
	} else {
		/* Shouldn't happen... */
		espstat.esps_crypto++;
...
#ifdef __QNXNTO__
	strlcpy(tc->tc_ifname, sav->if_outer, sizeof(tc->tc_ifname));
	/* Remember SA inner to find again on CB for scenarios where the SA
	 * got deleted during encryption.
	 */
#endif
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/lib/io-pkt/sys/netipsec/xform_esp.c $ $Rev: 680336 $")
#endif
